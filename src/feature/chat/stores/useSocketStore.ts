...
const readPinnedIds = (channelId: number): number[] => {
...
const writePinnedIds = (channelId: number, ids: number[]) => {
  if (typeof window === 'undefined') return;

  try {
    localStorage.setItem(getPinnedStorageKey(channelId), JSON.stringify(ids));
  } catch {
    //
  }
};

const syncPinnedIdsFromMessages = (channelId: number, messages: Message[]) => {
  const pinnedIds = messages.filter((m) => m.pinned).map((m) => m.id);
  writePinnedIds(channelId, pinnedIds);
};

const applyStoredPinnedState = (messages: Message[]): Message[] => {
  return messages.map((message) => {
    const storedPinnedIds = readPinnedIds(message.channelId);
    return {
      ...message,
      pinned: message.pinned || storedPinnedIds.includes(message.id),
    };
  });
};
...
export const useSocketStore = create<SocketState>((set, get) => ({
  ...
  connect: (token: string) => {
    ...
    socket.on('message:history', (data) => {
      ...
      const normalizedMessages: Message[] = applyStoredPinnedState(
      ...
      set((state) => {
        ...
        if (data.channelId) {
          syncPinnedIdsFromMessages(data.channelId, nextMessages);
        }
    ...
    socket.on('feedback:videos', (data) => {
      ...
      set((state) => {
        const nextMessages = sortByDate([...state.messages, ...videoMessages]);
        syncPinnedIdsFromMessages(data.channelId, nextMessages);
    ...
    socket.on('message', (incomingMessage: Message) => {
      const message = applyStoredPinnedState([
      ...
      set((state) => {
        ...
        syncPinnedIdsFromMessages(message.channelId, sorted);
    ...
    socket.on('message:deleted', (payload: { messageId: number }) => {
      set((state) => {
        const deletedMessage = state.messages.find(
        ...
        if (deletedMessage) {
          syncPinnedIdsFromMessages(deletedMessage.channelId, nextMessages);
        }
    ...
    socket.on('message:pinned', (payload: Message) => {
      ...
      set((state) => {
        ...
        syncPinnedIdsFromMessages(normalized.channelId, nextMessages);
  ...
  pinMessage: (channelId, messageId, pinned) => {
    ...
    set((state) => {
      ...
      syncPinnedIdsFromMessages(channelId, nextMessages);

...
export const MessageReactionPill = memo(
  ({ emojiId, count, isActive, isPending, onClick }: Props) => {
    const localOption = REACTION_OPTIONS.find(
      (option) => option.id === emojiId
    );
...
MessageReactionPill.displayName = 'MessageReactionPill';
